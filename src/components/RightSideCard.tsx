...
const RightSideCard: React.FC<RightSideProps> = (props) => {
    ...
    return (
        <div>
            ...
            <h4 className='capitalize font-medium text-[16px] md:text-xl mb-3 text-[#474747]'>{isEventType(rightSideData) ? rightSideData?.eventName : rightSideData?.name}</h4>
            <div>
                <p className='text-[13px] md:text-[15px] text-[#818181] textinThreeLine'>{isEventType(rightSideData) ? rightSideData?.description : rightSideData?.introduction}</p>
                {
                    !isEventType(rightSideData) && rightSideData?.role == 'PROFESSOR' &&   <div className='readMoreBtnContainer flex justify-end'>
                    <Button className='px-0 !pb-0 text-[14px] md:text-[15px]' label="Read Profie" link onClick={handlePopupData} />
                </div>
                }
